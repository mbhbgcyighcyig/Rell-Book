...
<head>
    ...
    <title>Nota Pengembalian - {{ $loan->loan_code }}</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nota {
            ...
            border-radius: 16px;
            overflow: hidden;
            box-shadow: 0 4px 24px rgba(139,94,60,.15);
        }
        ...
        .nota-title {
            ...
            font-weight: 700;
            color: #5c3d1e;
            border-bottom: 2px dashed #ede0cc;
        ...
        .denda-box.lunas { background: #f0fdf4; border: 1px solid #bbf7d0; }
        .denda-box.belum { background: #fef2f2; border: 1px solid #fecaca; }
        .denda-label { font-size: .78rem; font-weight: 600; }
        .denda-label.lunas { color: #166534; }
        .denda-label.belum { color: #991b1b; }
        .denda-amount { font-size: 1rem; font-weight: 800; }
        .denda-amount.lunas { color: #166534; }
        .denda-amount.belum { color: #dc2626; }

        /* Divider zigzag */
        .zigzag {
            height: 12px;
            background: repeating-linear-gradient(90deg, #f5efe6 0, #f5efe6 10px, #fff 10px, #fff 20px);
            margin: 1rem -2rem;
        }
...
<div class="nota-wrap">
    ...
    <div class="print-actions">
        ...
        <a href="{{ route('peminjam.returns.index') }}" class="btn-back">
            ← Kembali
        </a>
    </div>
    ...
    <div class="nota">
        ...
        <div class="nota-header">
            <img src="{{ asset('images/vitaww.jpeg') }}" alt="Logo" class="nota-logo">
            <div class="nota-org">Perpustakaan Digital</div>
            <div class="nota-sub">Sistem Manajemen Perpustakaan</div>
        </div>

        <div class="nota-body">
            <div class="nota-title">Nota Pengembalian Buku</div>

            <div class="nota-code">
                No. <span>{{ $loan->loan_code }}</span>
            </div>
            ...
            <div class="nota-section">
                ...
                <div class="nota-row">
                    ...
                    <span class="nota-val">{{ $loan->return_date->format('d M Y') }}</span>
                </div>
                <div class="nota-row">
                    <span class="nota-key">Durasi</span>
                    <span class="nota-val">{{ $loan->loan_date->diffInDays($loan->return_date) }} hari</span>
                </div>
                <div class="nota-row">
                    <span class="nota-key">Status</span>
                    <span class="nota-val"><span class="status-badge status-returned">Dikembalikan</span></span>
                </div>
                @if($loan->returnRequest?->confirmedBy)
                <div class="nota-row">
                    <span class="nota-key">Dikonfirmasi oleh</span>
                    <span class="nota-val">{{ $loan->returnRequest->confirmedBy->name }}</span>
                </div>
                <div class="nota-row">
                    <span class="nota-key">Waktu Konfirmasi</span>
                    <span class="nota-val">{{ $loan->returnRequest->confirmed_at->format('d M Y, H:i') }}</span>
                </div>
                @endif
            </div>
            ...
            <div class="nota-row" style="border-bottom:none">
                <span class="nota-key">Keterlambatan</span>
                <span class="nota-val">{{ $loan->fine_days }} hari</span>
            </div>
            <div class="denda-box {{ $loan->fine_paid ? 'lunas' : 'belum' }}">
                <div>
                    <div class="denda-label {{ $loan->fine_paid ? 'lunas' : 'belum' }}">
                        {{ $loan->fine_paid ? '✓ Denda Lunas' : '⚠ Denda Belum Dibayar' }}
                    </div>
                    <div style="font-size:.7rem;color:#9c7c5c">Rp 1.000 × {{ $loan->fine_days }} hari</div>
                </div>
                <div class="denda-amount {{ $loan->fine_paid ? 'lunas' : 'belum' }}">
                    Rp {{ number_format($loan->fine_amount, 0, ',', '.') }}
                </div>
            </div>
            @else
            <div class="denda-box lunas">
                <div class="denda-label lunas">✓ Tidak Ada Denda</div>
                <div class="denda-amount lunas">Rp 0</div>
            </div>
            @endif
        ...
        <div class="nota-footer">
            <div class="thanks">Terima kasih telah mengembalikan buku!</div>
            <p>
                Nota ini dicetak pada {{ now()->format('d M Y, H:i') }}<br>
                Simpan nota ini sebagai bukti pengembalian buku.<br>
